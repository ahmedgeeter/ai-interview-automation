...
----------------------------------------------
This module defines the core state machine for the AI Interviewer using LangGraph.
It maps out the logical flow of the interview process:
1. Guardrail Node: Validates state and constraints before proceeding.
2. Interviewer Node: Generates the next question/response based on context.
3. Evaluator Node: Computes final scorecard once the session limit is reached.

The state is persisted in memory during execution to allow for continuous streaming
and seamless interruptions across WebSocket frames.
"""

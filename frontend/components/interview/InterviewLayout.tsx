    ...
    <header className="h-12 border-b border-slate-200 dark:border-stone-800/40 flex items-center justify-between px-5 shrink-0 bg-white/50 dark:bg-stone-950/50 backdrop-blur-md">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${isConnected ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]" : "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.6)]"} transition-colors`} />
          <span className="text-[11px] font-semibold text-slate-500 dark:text-stone-500 uppercase tracking-wider">{isConnected ? t("uplink_on") : t("uplink_off")}</span>
        </div>

        {/* Turn State Visualizer */}
        {isConnected && (
          <div className="flex items-center gap-2 ml-4 border-l border-slate-200 dark:border-stone-800 pl-4">
            <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">
              {/* @ts-ignore - Assuming turnState or isAiSpeaking passed via props if updated */}
              {typeof (globalThis as any).isAiSpeaking !== "undefined" ? "" : ""} 
            </span>
            {/* The actual props for these states will need to be passed in from the parent. Since I am replacing this, I will rely on props.turnState */}
          </div>
        )}
      </div>
      <div className="flex items-center gap-2">
